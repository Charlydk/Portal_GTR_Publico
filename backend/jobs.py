...
async def poblado_diario_bolsa_reporteria():
    """Busca las plantillas activas y genera la bolsa para el día."""
    try:
        async with AsyncSessionLocal() as db:
            ...
            q_plantillas = select(models.CatalogoTareasReporteria).filter(models.CatalogoTareasReporteria.activa == True)
            ...
            for p in plantillas:
                nueva_tarea = models.BolsaTareasReporteria(
                    ...
                    nombre=p.nombre,
                    descripcion=f"Generado automáticamente desde {p.nombre}",
            ...
            await db.commit()
            print(f"Bolsa diaria de reportería generada: {len(plantillas)} tareas.")
